...
complete -o bashdefault -o default -o nospace -F _cli_bash_autocomplete ghomfc`)
	return nil
}
...
func (cc *completionCommand) zshCompletionAction(context.Context, *cli.Command) error {
	...
	fmt.Fprintln(cc.stdout, `#compdef ghomfc
...
if [ "$funcstack[1]" = "_ghomfc" ]; then
  _ghomfc "$@"
else
  compdef _ghomfc ghomfc
fi`)
	return nil
}
